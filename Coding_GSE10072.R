...
up_genes <- subset(volcano_data, status == "UP")
down_genes <- subset(volcano_data, status == "DOWN")
...
heatmap_data <- exprs_data[rownames(top50), ]
...
heatmap_data <- exprs_data[top50$PROBEID, ]


#Analisis Enrichment (GO & KEGG)
...
# Significant Gen
sig_genes <- subset(topTableResults, adj.P.Val < 0.01)
